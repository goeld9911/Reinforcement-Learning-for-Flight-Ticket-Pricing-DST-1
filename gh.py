...
def run():
    ...
    if param_ascend == "Low to High":
        ...
        m = folium.Map(tiles=add_select, location=[latitude, longitude], zoom_start=10)

        # add marker for Indian Cities
        # tooltip = df["Dept_city"]
        folium.Marker([latitude, longitude], popup=dcity, tooltip=f"Departure city {dcity} to Arrival City {city}").add_to(m)
...
with st.sidebar.subheader("Select Background for Map"):
    add_select = st.sidebar.selectbox("Background",options=["OpenStreetMap", "Stamen Terrain","Stamen Toner"])
